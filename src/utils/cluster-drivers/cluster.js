...
function findClosestAvailableDriver(point, drivers, driverLoads) {
    let closestDriver = null;
    let minDistance = Infinity;
    for (const driver of drivers) {
        const currentLoad = driverLoads.get(driver.id) || 0;
        if (currentLoad >= driver.capacity)
            continue; // Skip drivers who are full
...
function assignParentsToDrivers(locations, drivers) {
    var _a;
    ...
    for (let i = 0; i < locations.length; i++) {
        ...
        const closestDriverId = findClosestAvailableDriver(parentLocation, drivers, driverLoads);
        if (closestDriverId !== null) {
            (_a = assignments.get(closestDriverId)) === null || _a === void 0 ? void 0 : _a.push(i); // Assign parent to driver
            driverLoads.set(closestDriverId, (driverLoads.get(closestDriverId) || 0) + 1); // Increment driver's load
        }
        else {
            console.log(`No available drivers for parent at index ${i} (Location: ${parentLocation})`);
        }
    }
    return assignments;
}
...
console.log('Carpool assignments (driverId -> parent indices):', carpoolAssignments);
